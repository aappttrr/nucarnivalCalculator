...
class SRRei(SRCard):
    ...
    def passive_star_3(self):
        if super(SRRei, self).passive_star_3():
            count = 0
            ...
            if count > 0:
                buff = Buff('SRRei_passive_star_3', 0.08 * count, 0, BuffType.AtkIncrease)
                buff.isPassive = True
                self.addBuff(buff)

    # 攻+25%
    def passive_star_5(self):
        if super(SRRei, self).passive_star_5():
            buff = Buff('SRRei_passive_star_5', 0.25, 0, BuffType.AtkIncrease)
            buff.isPassive = True
            self.addBuff(buff)

    # 攻+10%
    def passive_tier_6(self):
        if super(SRRei, self).passive_tier_6():
            buff = Buff('SRRei_passive_tier_6', 0.1, 0, BuffType.AtkIncrease)
            buff.isPassive = True
            self.addBuff(buff)
